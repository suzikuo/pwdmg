...
function Show-Help {
    ...
    Write-Host "  2. Updates tauri.conf.json, Cargo.toml, front package versions, Android versionName/versionCode, and front manifest version."
...
$TauriConf = Join-Path $RootPath "src-tauri\tauri.conf.json"
$TauriCargo = Join-Path $RootPath "src-tauri\Cargo.toml"
$CoreCargo = Join-Path $RootPath "src-tauri\crates\core\Cargo.toml"
$HostCargo = Join-Path $RootPath "src-tauri\crates\native_host\Cargo.toml"
...
$ManifestScript = Join-Path $RootPath "scripts\release\write_update_manifest.ps1"
...
if ($GenerateSigningKey) {
    $SignScript = Join-Path $PSScriptRoot "sign_updater.js"
    node $SignScript generate-signing-key --private-key $SigningPrivateKeyPath --public-key $SigningPublicKeyPath
...
function Get-CurrentVersion {
    $Text = Read-Text $TauriConf
    $Match = [regex]::Match($Text, '"version"\s*:\s*"(?<version>\d+\.\d+\.\d+)"')
...
function Ensure-AndroidSigningEnvironment {
    ...
    if (-not (Test-Path -LiteralPath $env:MYPWDMG_ANDROID_KEYSTORE -PathType Leaf)) {
        if (-not [Console]::IsInputRedirected) {
            Write-Host ""
            Write-Host "Android release keystore was not found at: $env:MYPWDMG_ANDROID_KEYSTORE" -ForegroundColor Yellow
            Write-Host "Options:"
            Write-Host "  [1] Generate a new Android release keystore now (using keytool)"
            Write-Host "  [2] Skip Android and build Desktop only"
            Write-Host "  [3] Specify an existing keystore path"
            $KeystoreChoice = (Read-Host "Choose 1, 2, or 3").Trim()
            if ($KeystoreChoice -eq "1") {
                $Keytool = Resolve-KeytoolPath
                if (-not $Keytool) {
                    throw "keytool.exe was not found. Set JAVA_HOME or install Java to generate a keystore."
                }
                $NewPass = Read-SecretText "Enter password for new keystore (at least 6 characters)"
                if ($NewPass.Length -lt 6) {
                    throw "Keystore password must be at least 6 characters."
                }
                $NewAlias = (Read-Host "Enter key alias (default: mypwdmg)").Trim()
                if (-not $NewAlias) { $NewAlias = "mypwdmg" }
                $DName = "CN=My Password, OU=Mobile, O=Suzikuo, C=CN"
                Write-Host "Generating Android release keystore at $env:MYPWDMG_ANDROID_KEYSTORE..." -ForegroundColor Cyan
                & $Keytool -genkeypair -v -keystore $env:MYPWDMG_ANDROID_KEYSTORE -alias $NewAlias -keyalg RSA -keysize 2048 -validity 10000 -storepass $NewPass -keypass $NewPass -dname $DName
                if ($LASTEXITCODE -eq 0 -and (Test-Path $env:MYPWDMG_ANDROID_KEYSTORE)) {
                    Write-Host "Keystore created successfully!" -ForegroundColor Green
                    $env:MYPWDMG_ANDROID_KEYSTORE_PASSWORD = $NewPass
                    $env:MYPWDMG_ANDROID_KEY_ALIAS = $NewAlias
                    $env:MYPWDMG_ANDROID_KEY_PASSWORD = $NewPass
                    return
                } else {
                    throw "Failed to generate Android keystore with keytool."
                }
            } elseif ($KeystoreChoice -eq "2") {
                Write-Host "Skipping Android packaging; proceeding with Desktop only." -ForegroundColor Yellow
                $script:ShouldIncludeAndroid = $false
                $script:DesktopOnly = $true
                return
            } elseif ($KeystoreChoice -eq "3") {
                $CustomPath = (Read-Host "Enter full path to your .jks/.keystore file").Trim().Trim('"')
                if (Test-Path -LiteralPath $CustomPath -PathType Leaf) {
                    $env:MYPWDMG_ANDROID_KEYSTORE = (Resolve-Path $CustomPath).Path
                } else {
                    throw "Keystore file not found at: $CustomPath"
                }
            } else {
                throw "Android signing keystore was not found: $env:MYPWDMG_ANDROID_KEYSTORE"
            }
        } else {
            throw "Android signing keystore was not found: $env:MYPWDMG_ANDROID_KEYSTORE. Place your release keystore at this path, pass -DesktopOnly to package Windows only, or generate a keystore using keytool."
        }
    }
...
function Update-VersionFiles {
    ...
    Update-RegexFile $TauriConf '"version"\s*:\s*"\d+\.\d+\.\d+"' "`"version`": `"$NextVersion`""
    Update-RegexFile $TauriCargo '(?m)^version\s*=\s*"\d+\.\d+\.\d+"' "version = `"$NextVersion`""
    Update-RegexFile $CoreCargo '(?m)^version\s*=\s*"\d+\.\d+\.\d+"' "version = `"$NextVersion`""
    Update-RegexFile $HostCargo '(?m)^version\s*=\s*"\d+\.\d+\.\d+"' "version = `"$NextVersion`""
...
function Get-FrontendBuildMarker {
    param(
        [string] $DistDir,
        [string] $Target
    )
    ...
    $Pattern = "\d+\.\d+\.\d+-$Target-\d{17}"
    foreach ($File in (Get-ChildItem -LiteralPath $DistDir -Recurse -File -Filter "*.js")) {
...
function Invoke-AndroidFrontendPackage {
    Write-Host "==> Building Android frontend assets..." -ForegroundColor Cyan
    Push-Location $FrontDir
    try {
        npm run build:android | Out-Host
        Assert-LastExitCode "Android frontend build"
    }
    finally {
        Pop-Location
    }

    $AndroidDistDir = Join-Path $FrontDir "dist\android"
    $BuildMarker = Get-FrontendBuildMarker $AndroidDistDir "android"
    if (-not $BuildMarker) {
        throw "Android frontend build marker was not found in $AndroidDistDir. The Android APK must not be built from unmarked frontend assets."
    }
    Write-Host "Android frontend build marker: $BuildMarker" -ForegroundColor Green
    return $BuildMarker
}

function Get-ArchiveFrontendBuildMarker {
    param(
        [string] $ArchivePath,
        [string] $Target
    )
    ...
    try {
        foreach ($Entry in ($Archive.Entries | Where-Object { $_.FullName -match '\.js$' })) {
            ...
            try {
                ...
                try {
                    $Match = [regex]::Match($Reader.ReadToEnd(), $Pattern)
                    if ($Match.Success) {
                        return $Match.Value
                    }
                }
...
function Invoke-AndroidPackage {
    ...
    $AndroidFrontendMarker = ""
    if (-not $SkipAndroidBuild) {
        $AndroidFrontendMarker = Invoke-AndroidFrontendPackage
        ...
        Write-Host "Skipping Android frontend/Gradle build; using the existing APK and its existing frontend marker." -ForegroundColor Yellow
    }
    ...
    $ApkFrontendMarker = Get-ArchiveFrontendBuildMarker $AndroidReleaseOutput "android"
    if (-not $ApkFrontendMarker) {
        throw "Android APK does not contain a frontend build marker. Refusing to publish an APK that cannot prove which frontend was packaged."
    }
    if ($AndroidFrontendMarker -and $ApkFrontendMarker -ne $AndroidFrontendMarker) {
        throw "Android APK frontend marker mismatch. Built frontend: $AndroidFrontendMarker; APK: $ApkFrontendMarker"
    }
    Write-Host "Android APK frontend build marker: $ApkFrontendMarker" -ForegroundColor Green
...
function Assert-SignedManifest {
    ...
    $SignScript = Join-Path $PSScriptRoot "sign_updater.js"
    node $SignScript verify-manifest --input $Path --public-key $SigningPublicKeyPath
...
function Publish-GitHubRelease {
    ...
    $SetupExePath = Join-Path $RootPath "release\My Password-Setup.exe"
